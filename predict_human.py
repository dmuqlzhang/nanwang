...
def box_iof(box1, box2, iof_th):
    ...
    def tower_adj(box):
        # box = 4xn
        return box[3] - 1/12*(box[3] - box[1])
    
    def person_top(box):
        # box = 4xn
        return box[3] - 1/4*(box[3] - box[1])

    area1 = box_area(box1.T)
    area2 = box_area(box2.T)

    tower_adjDown = tower_adj(box2.T)
    person_Down = person_down(box1.T)
    tower_Down = person_down(box2.T)
    person_Top = person_down(box1.T)
    person_in_down = tower_adjDown - person_Down[:, None]
    person_in_top = tower_adjDown - person_Top[:, None]

    is_down = person_in_down > 0
    is_top = person_in_top > 0


    # inter(N,M) = (rb(N,M,2) - lt(N,M,2)).clamp(0).prod(2)
    inter = (torch.min(box1[:, None, 2:], box2[:, 2:]) - torch.max(box1[:, None, :2], box2[:, :2])).clamp(0).prod(2)
    iof = inter / torch.min(area1[:, None] , area2) > iof_th

    for id, i in enumerate(is_top):
        if False in i and True in i:
            is_top[id] = False
    

    # print(is_top)

    return is_down*iof + is_top
...
def postprocess(det, iof_th):
    towers = det[det[:, -1] < 3]
    persons = det[det[:, -1] ==3]
    if not len(towers) or not len(persons):
        return None
    towers_flag = box_iof(persons[:, :4], towers[:, :4], iof_th)
    persons = persons[torch.max(towers_flag, dim=1)[0] > 0]
...
class Detector:
    ...
    def __init__(self, model_path, img_size=416, conf_thres=0.25, iou_thres=0.2, device='',
        ...
        self.names = ['poletower', 'tower','tower_2','person','head_helmet',"uniform","head",'helmet']
        print(self.names)
    ...
    def detect(self, img0_file):
        ...
        ##judge
        human_det = postprocess(det, 0.5)

        ##
        all_result = []
        if human_det is None or len(human_det) == 0:
            #cv2.imwrite('/data/zhangqinlu/workspace/nanwang_comptetion/my_scripts/yolov5_infer/data/' + img0_file.split('/')[-1],img0)    
            return {}
        for *xyxy, conf, cls in human_det:
            label =  'human'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, help='model path', default='/data/zhangqinlu/workspace/nanwang_comptetion/my_scripts/yolov5_nanwang_infer_code/weights/human/epoch130.pt')
    parser.add_argument('--img_dir', type=str,  default='/data/zhangqinlu/workspace/nanwang_comptetion/my_scripts/yolov5_nanwang_infer_code/data/rr.jpg',help='image dir')
    ...
    parser.add_argument('--conf_thres', type=float, default=0.4, help='conf')
